...
async def test_default_control_system_import():
    ...
    """
    assert bool(await load("VMX"))
    assert isinstance((await load("VMX"))._cs, pytac.aioca_cs.AIOCAControlSystem)


async def test_import_fail_raises_ControlSystemException(mock_cs_raises_ImportError):
    """In this test we:
    ...
    """
    with patch("pytac.aioca_cs.AIOCAControlSystem", mock_cs_raises_ImportError):
