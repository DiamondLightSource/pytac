...
async def test_get_values_live(simple_epics_lattice, mock_cs):
    await simple_epics_lattice.get_element_values("family", "x", pytac.RB, pytac.PHYS)
    mock_cs.get_multiple.assert_called_with([RB_PV], True)


async def test_get_values_sim(simple_epics_lattice):
    mock_ds = mock.AsyncMock(units=pytac.PHYS)
    mock_uc = mock.Mock()
    simple_epics_lattice[0].set_data_source(mock_ds, pytac.SIM)
    simple_epics_lattice[0].set_unitconv("a_field", mock_uc)
    await simple_epics_lattice.get_element_values(
...
async def test_set_element_values_live(simple_epics_lattice, mock_cs):
    await simple_epics_lattice.set_element_values("family", "x", [1], units=pytac.PHYS)
    mock_cs.set_multiple.assert_called_with([SP_PV], [1], True)


async def test_set_element_values_sim(simple_epics_lattice):
    mock_ds = mock.AsyncMock(units=pytac.PHYS)
    mock_uc = mock.Mock()
    mock_uc.convert.return_value = 1
    simple_epics_lattice[0].set_data_source(mock_ds, pytac.SIM)
    simple_epics_lattice[0].set_unitconv("a_field", mock_uc)
    await simple_epics_lattice.set_element_values(
...
async def test_set_element_values_raises_correctly(simple_epics_lattice):
    with pytest.raises(IndexError):
        await simple_epics_lattice.set_element_values("family", "x", [1, 2])
    with pytest.raises(IndexError):
        await simple_epics_lattice.set_element_values(
...
async def test_get_values_returns_numpy_array_if_requested(
    simple_epics_lattice, dtype, expected, mock_cs
):
    values = await simple_epics_lattice.get_element_values(
...
async def test_get_value_uses_cs_if_data_source_live(simple_epics_element, mock_cs):
    await simple_epics_element.get_value("x", handle=pytac.SP, data_source=pytac.LIVE)
    mock_cs.get_single.assert_called_with(SP_PV, True)
    await simple_epics_element.get_value("x", handle=pytac.RB, data_source=pytac.LIVE)
    mock_cs.get_single.assert_called_with(RB_PV, True)


async def test_get_value_raises_handle_exceptions(simple_epics_element):
    with pytest.raises(pytac.exceptions.HandleException):
        await simple_epics_element.get_value("y", "unknown_handle")
...
async def test_set_element_values_length_mismatch_raises_index_error(
    simple_epics_lattice,
):
    with pytest.raises(IndexError):
        await simple_epics_lattice.set_element_values("family", "x", [1, 2])
    with pytest.raises(IndexError):
        await simple_epics_lattice.set_element_values("family", "x", [])


async def test_element_get_pv_name_raises_exceptions(simple_epics_element):
    with pytest.raises(pytac.exceptions.FieldException):
        await simple_epics_element.get_pv_name("unknown_field", "setpoint")
    basic_epics_element = simple_epics_element
    with pytest.raises(pytac.exceptions.DataSourceException):
        await basic_epics_element.get_pv_name("basic", pytac.RB)
    del basic_epics_element._data_source_manager._data_sources[pytac.LIVE]
    with pytest.raises(pytac.exceptions.DataSourceException):
        await basic_epics_element.get_pv_name("x", pytac.RB)
